...
#define SERIAL_STATUS 0x80

// Serial read modes
#define SERIAL_READ_CONTINUOUSLY 0x00
#define SERIAL_STOP_READING 0x01
#define SERIAL_IS_OPEN 0x02
#define SERIAL_IS_CLOSE 0x03
...
namespace FirmataSerial {
    ...
    inline uint8_t getSerialPinType(uint8_t pin) {
        ...
        if (pin == PIN_SERIAL1_RX)
            return RES_RX1;
        if (pin == PIN_SERIAL1_TX)
            return RES_TX1;
#endif
#if defined(PIN_SERIAL2_RX)
        if (pin == PIN_SERIAL2_RX)
            return RES_RX2;
        if (pin == PIN_SERIAL2_TX)
            return RES_TX2;
#endif
#if defined(PIN_SERIAL3_RX)
        if (pin == PIN_SERIAL3_RX)
            return RES_RX3;
        if (pin == PIN_SERIAL3_TX)
            return RES_TX3;
#endif
#if defined(PIN_SERIAL4_RX)
        if (pin == PIN_SERIAL4_RX)
            return RES_RX4;
        if (pin == PIN_SERIAL4_TX)
            return RES_TX4;
#endif
#if defined(PIN_SERIAL5_RX)
        if (pin == PIN_SERIAL5_RX)
            return RES_RX5;
        if (pin == PIN_SERIAL5_TX)
            return RES_TX5;
#endif
#if defined(PIN_SERIAL6_RX)
        if (pin == PIN_SERIAL6_RX)
            return RES_RX6;
        if (pin == PIN_SERIAL6_TX)
            return RES_TX6;
#endif
#if defined(PIN_SERIAL7_RX)
        if (pin == PIN_SERIAL7_RX)
            return RES_RX7;
        if (pin == PIN_SERIAL7_TX)
            return RES_TX7;
#endif
        return 0;
    }
...
public:
    ...
    bool handlePinMode(mFirmata *, byte pin, int mode);

    void handleCapability(mFirmata *, nStream *, byte pin);

    bool handleSysex(mFirmata *fm, nStream *, byte command, byte argc, byte *argv);

    void update(mFirmata *fm, nStream *FirmataStream);

    void reset();

    void checkSerial(mFirmata *fm, nStream *FirmataStream);
...
private:
    ...
    kSerial *getPortFromId(byte portId);

    bool IS_PIN_SERIAL(byte pin);
};
