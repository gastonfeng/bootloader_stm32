...
#ifdef USE_FIRMATA_WIRE
void wireWrite(byte data) { Wire.write((byte)data); }

byte wireRead() { return Wire.read(); }
...
void mFirmata::reportAnalogCallback(nStream *stream, byte analogPin,
                                    int value) {
#if defined(RTE_APP) || defined(PLC)
...
void attachServo(mFirmata *fm, Stream *fs, byte pin, int minPulse,
                 int maxPulse) {
...
void mFirmata::report(nStream *FirmataStream) {
  ...
  if (currentMillis - previousMillis > rte_config.reportInterval) {
    ...
    board.readAnalogValue(this, FirmataStream,
                          rte_data.firmata.analogInputsToReport,
  ...
  //         readAndReportData(Firmata, query[i].addr, query[i].reg,
  //         query[i].bytes, query[i].stopTX);
  //     }
  // }
}
...
void mFirmata::outputPort(nStream *FirmataStream, byte portNumber,
                          byte portValue, byte forceSend) {
    // pins not configured as INPUT are cleared to zeros
    //    portValue = portValue & portConfigInputs[portNumber];
    // only send if the value is different than previously sent
    if (portNumber < (IO_XI_NRS + IO_YO_NRS) &&
        (forceSend ||
         bitRead(rte_data.firmata.previousPINs, portNumber) != portValue)) {
...
int mFirmata::setValue(nStream *FirmataStream, int index, void *valBuf,
                       u8 size) {
    byte *buf;
    ...
    *(int *) buf = index;
    ...
    sendSysex(FirmataStream, pb_firmata_cmd_FM_WRITE_VALUE, size + 4,
              (byte *) buf);
    free(buf);
...
int mFirmata::get_flag(u16 cmd) { return respose[cmd / 8] & (1 << (cmd % 8)); }
...
int mFirmata::getPinState(byte pin) { return rte_data.X[pin]; }

void mFirmata::setPinState(byte pin, int state) { rte_data.X[pin] = state; }

void mFirmata::encodeByteStream(nStream *FirmataStream, size_t bytec,
                                uint8_t *bytev, size_t max_bytes) {
    if (FirmataStream == nullptr) {
        return;
    }
    // logger.debug("encodeByteStream:%p bytec=%d,bytev=%p, max_bytes=%d",
    // FirmataStream, bytec, bytev, max_bytes);
    ...
    if (bytec > max_bytes) {
        // logger.error("encodeByteStream: bytec %d > max_bytes %d", bytec,
        // max_bytes);
        return;
    }
    for (size_t i = 0; (i < bytec) && (bytes_sent < max_bytes); ++i) {
        auto transmit_byte =
                (uint8_t) (outstanding_bit_cache | (bytev[i] << outstanding_bits));
...
void mFirmata::marshaller_sendSysex(nStream *FirmataStream, uint8_t command,
...
void mFirmata::sendSysex(nStream *FirmataStream, byte command, uint16_t bytec,
                         byte *bytev, bool _crc_en) {
    crc_en = _crc_en || crc_en;
    if (lock)
        Rtos::mutex_lock(lock);
    if (FirmataStream->tx_lock)
        Rtos::mutex_lock(FirmataStream->tx_lock);
    //    if (s != 0)
    ...
    //    s = 0;
    if (FirmataStream->tx_lock)
        Rtos::mutex_unlock(FirmataStream->tx_lock);
    if (lock)
...
#ifdef USE_FREERTOS
    int inputData =
        FirmataStream->read_wait(-1); // this is 'int' to handle -1 when no data
#else
    int inputData =
...
void mFirmata::sendDigitalPort(nStream *FirmataStream, uint8_t portNumber,
                               uint16_t portData) {
    FirmataStream->write(DIGITAL_MESSAGE | (portNumber & 0xF));
    // Tx bits  0-6 (protocol v1 and higher)
    // Tx bits 7-13 (bit 7 only for protocol v2 and higher)
    encodeByteStream(FirmataStream, sizeof(portData), (uint8_t *) (&portData),
                     sizeof(portData));
}
            ...
            switch (executeMultiByteCommand) {
                ...
                case ANALOG_MESSAGE:
                  if (currentAnalogCallback) {
                    (*currentAnalogCallback)(
                        currentAnalogCallbackContext, stream, multiByteChannel,
                ...
                case DIGITAL_MESSAGE:
                  if (currentDigitalCallback) {
                    (*currentDigitalCallback)(
                        currentDigitalCallbackContext, stream, multiByteChannel,
                        (uint16_t)(dataBuffer[0] << 7) + dataBuffer[1]);
                  }
                  break;
                case SET_PIN_MODE:
                  if (currentPinModeCallback)
                    (*currentPinModeCallback)(currentPinModeCallbackContext, stream,
                                              dataBuffer[1], dataBuffer[0]);
                  break;
                case SET_DIGITAL_PIN_VALUE:
                  if (currentPinValueCallback)
                    (*currentPinValueCallback)(currentPinValueCallbackContext, stream,
                                               dataBuffer[1], dataBuffer[0]);
                  break;
                case REPORT_ANALOG:
                  if (currentReportAnalogCallback)
                    (*currentReportAnalogCallback)(currentReportAnalogCallbackContext,
                                                   stream, multiByteChannel,
                                                   dataBuffer[0]);
                  break;
                case REPORT_DIGITAL:
                  if (currentReportDigitalCallback)
                    (*currentReportDigitalCallback)(currentReportDigitalCallbackContext,
                                                    stream, multiByteChannel,
                                                    dataBuffer[0]);
                  break;
#endif
...
void mFirmata::processSysexMessage(nStream *stream) {
    ...
    switch (dataBuffer[0]) {
        ...
        case STRING_DATA:
            // const size_t end_of_string = (string_offset +
            // decodeByteStream((sysexBytesRead - string_offset),
            //                                                                &dataBuffer[string_offset]));
            // bufferDataAtPosition('\0', end_of_string); // nullptr terminate the
            // string
        ...
        default:
            if (sysexBytesRead > 1) {
                ...
                int len = decodeByteStream(sysexBytesRead - 1, &dataBuffer[1],
                                           dataBufferDecode);
...
bool mFirmata::bufferDataAtPosition(nStream *stream, const uint8_t data,
                                    const size_t pos) {
...
bool mFirmata::getPinMode(byte i) { return false; }
...
void mFirmata::sysexCallback(nStream *FirmataStream, byte command,
                             uint16_t argc, byte *argv) {
    int index;
    int state;
    uint32_t start, end;
    uint32_t indexv;
    ...
    switch (command) {
        ...
        case pb_firmata_cmd_CB_GET_RTE_VERSION:
            sendSysex(FirmataStream, pb_firmata_cmd_CB_GET_RTE_VERSION,
                      sizeof(pb_rte_const), (uint8_t *) &rteConst);
            break;
            ...
            case pb_firmata_cmd_CB_GET_REMAIN_MEM:
                sendSysex(FirmataStream, pb_firmata_cmd_CB_GET_REMAIN_MEM, 2,
                          (byte *) &rte.data.remain_mem);
                break;
#if defined(RTE_APP) || defined(PLC)
            ...
            case pb_firmata_cmd_CB_GET_RTC:
                sendSysex(FirmataStream, pb_firmata_cmd_CB_GET_RTC, sizeof(pb_rtc_info),
                          (byte *) &rtc.data);
                break;
            ...
            case pb_firmata_cmd_CB_SET_IP:
              ...
              ETH_LWIP::set_ip();
              sendSysex(FirmataStream, pb_firmata_cmd_CB_SET_IP, 4,
                        (byte *)(&rte_config.lwip.ip));
              break;
#endif
            case pb_firmata_cmd_CB_GET_IP:
              sendSysex(FirmataStream, pb_firmata_cmd_CB_GET_IP, 4,
                        (byte *)(&rte_config.lwip.ip));
              break;
            case pb_firmata_cmd_FM_GET_NET_BUF_STAT: {
              ...
              sendSysex(FirmataStream, pb_firmata_cmd_FM_GET_NET_BUF_STAT, 13 * MEMP_MAX,
                        (byte *)buffer);
              free(buffer);
            } break;
#endif
            ...
            case pb_firmata_cmd_CB_SET_FORCE:
              for (int i = 0; i < argc;) {
                ...
                if (rte.data.state == pb_state_Started) {
                  ((plc_app_abi_t *)app.data.plc_curr_app)
                      ->dbg_set_force(index, len ? &argv[i + 3] : nullptr);
                }
                i += len + 3;
              }
            ...
            case pb_firmata_cmd_CB_CLEAR_V:
              ...
              if (rte.data.state == pb_state_Started) {
                ((plc_app_abi_t *)app.data.plc_curr_app)
                    ->dbg_vars_reset(__IEC_DEBUG_FLAG);
            ...
            case pb_firmata_cmd_CB_GET_V: {
              ...
              if (rte.data.state == pb_state_Started) {
                void *b = nullptr;
                ((plc_app_abi_t *)app.data.plc_curr_app)
                    ->dbg_data_get((uint32_t *)&data[0], (uint32_t *)&len, (void **)&b);
        ...
        case pb_firmata_cmd_CB_SET_SERIAL_TX_LOW:
            ...
            case pb_firmata_cmd_FM_GET_TASK_NAME:
              if (tasks[argv[0]]) {
                sendSysex(FirmataStream, pb_firmata_cmd_FM_GET_TASK_NAME,
                          (byte)strlen(tasks[argv[0]]->task_name),
                          (byte *)tasks[argv[0]]->task_name);
              }
              break;
            case pb_firmata_cmd_FM_GET_TASK_DETAIL:
              if (argv[0] < PLC_TASK_NRS && tasks[argv[0]]) {
                sendSysex(FirmataStream, pb_firmata_cmd_FM_GET_TASK_DETAIL, 24,
                          tasks[argv[0]]->mata());
              }
              break;
            case pb_firmata_cmd_FM_GET_PLC_INFO:
              if (app.data.plc_curr_app) {
                ...
                sendSysex(FirmataStream, pb_firmata_cmd_FM_GET_PLC_INFO, sizeof(info),
                          (byte *)&info);
              } else
                sendSysex(FirmataStream, pb_firmata_cmd_FM_GET_PLC_INFO, 0,
                          (byte *)&info);
              break;
#endif
        case pb_firmata_cmd_FM_GET_PLC_STATE:
            sendSysex(FirmataStream, pb_firmata_cmd_FM_GET_PLC_STATE, 1,
                      (byte *) (&rte.data.state));
            break;
        case pb_firmata_cmd_CB_GET_LOG_NUMBER:
            sendSysex(FirmataStream, pb_firmata_cmd_CB_GET_LOG_NUMBER, 5,
                      (byte *) (&rte.data.state));
            break;
        case pb_firmata_cmd_CB_GET_LOG:
            ...
            case pb_firmata_cmd_FM_LIST_KEY:
              kfs.dir_buf(pb_firmata_cmd_FM_LIST_KEY, (const char *)&argv[8], 0, 16, this,
                          FirmataStream);
              break;
            case pb_firmata_cmd_CB_READ_KEY:
              ...
              sendSysex(FirmataStream, pb_firmata_cmd_CB_READ_KEY, vlen,
                        (byte *)value_crk);
              free(value_crk);
              break;
            case pb_firmata_cmd_FM_READ_KEY_BYTES:
              ...
              sendSysex(FirmataStream, pb_firmata_cmd_FM_READ_KEY_BYTES, len + 4,
                        (byte *)data);
              free(data);
              break;
            case pb_firmata_cmd_CB_WRITE_KEY:
              ...
              if ((key_len < FDB_KV_NAME_MAX) && (rw < FDB_STR_KV_VALUE_MAX_SIZE)) {
                rw = kvdb.set((const char *)argv, (const char *)argv + key_len + 1,
                              (int)rw, KV_STR_VALUE);
                buffer = (byte *)kvdb.get((const char *)argv);
                vlen = strlen((const char *)buffer);
            ...
            case pb_firmata_cmd_FM_WRITE_KEY_BYTES:
              ...
              sendSysex(FirmataStream, pb_firmata_cmd_FM_WRITE_KEY_BYTES, 2,
                        (byte *)&len);
              break;
            ...
            case pb_firmata_cmd_CB_SET_TSL_RANGE:
              ...
              sendSysex(FirmataStream, pb_firmata_cmd_CB_SET_TSL_RANGE, sizeof(tsl_query),
                        (byte *)&q);
              break;
            ...
            //     TSDB *tsdb = TSDB::db(db);
            //     if (tsdb) {
            //         tlen = tsdb->query_read((uint32_t *) &tbuf[1], (fdb_time_t *)
            //         &tbuf[6],
            //                                 (int *) (tbuf + 10),
            //                                 tbuf + 14, buf_sz - 14);
            //     }
            ...
            case pb_firmata_cmd_CB_GET_TSL_BY_ID: {
              ...
              *(short *)tbuf = 0;
              ...
              sendSysex(FirmataStream, pb_firmata_cmd_CB_GET_TSL_BY_ID, tlen,
                        (byte *)tbuf);
              free(tbuf);
            } break;
        ...
        case pb_firmata_cmd_CB_CPU_USAGE:
            sendSysex(FirmataStream, pb_firmata_cmd_CB_CPU_USAGE, 1,
                      (byte *) &rte.data.cpu_usage);
            break;
#ifdef USE_WIFI
            case CB_WIFI_LIST:
              sendSysex(FirmataStream, CB_WIFI_LIST, rte_data.wifi_size,
                        (byte *)&rte_data.wifi_list);
              break;
        ...
        case pb_firmata_cmd_FM_PUT_DATA_BLOCK: {
            ...
            if (block == 0) {
                ...
                if (dev) {
                    ...
                    //  logger.info("recv %d ,size= %d 0x%x 0x%x", block, argc - 4, argv[4],
                    //  argv[5]);
                }
            }
        }
            sendSysex(FirmataStream, pb_firmata_cmd_FM_PUT_DATA_BLOCK, 4,
                      (byte *) &state);
            break;
        case pb_firmata_cmd_FM_GET_DATA_BLOCK: {
            ...
            if (block == 0) {
                ...
                if (!dev) {
                    ...
                    *(int *) &tbuf[0] = 0;
                    *(int *) &tbuf[4] =
                            dev->begin_read(&argv[16], argc - 16, data_address, data_len);
                    blocksize = (FirmataStream->tx_max_size() * 7 / 8 - 16) & (~0x3);
                ...
                if (dev) {
                    ...
                    *(int *) &tbuf[0] = block;
                    *(uint32_t *) &tbuf[4] =
                            dev->Read(0, blocksize * (block - 1), blocksize, &tbuf[8]);
                    if (*(uint32_t *) &tbuf[4] < blocksize) {
                        dev->Shutdown();
                        dev = nullptr;
                    }
                    sendSysex(FirmataStream, pb_firmata_cmd_FM_GET_DATA_BLOCK,
                              *(uint32_t *) &tbuf[4] + 8, tbuf);
                    free(tbuf);
                    break;
                }
            }
            sendSysex(FirmataStream, pb_firmata_cmd_FM_GET_DATA_BLOCK, 4,
                      (byte *) &state);
            break;
        }
#endif
            ...
            case pb_firmata_cmd_FM_GET_LOC_TAB:
              ...
              if (argc == 4) {
                ...
                if (app.data.plc_curr_app &&
                    l_index < ((plc_app_abi_t *)app.data.plc_curr_app)->l_sz) {
                  plc_loc_tbl_t loc =
                      ((plc_app_abi_t *)app.data.plc_curr_app)->l_tab[l_index];
                  ...
                  *(u16 *)&buffer[4] = loc->a_size;
                  ...
                  sendSysex(FirmataStream, pb_firmata_cmd_FM_GET_LOC_TAB, len,
                            (byte *)buffer);
                  free(buffer);
                  break;
                }
            ...
            case pb_firmata_cmd_FM_SET_LOC_TAB:
              if (argc == 5) {
                ...
                if (app.data.plc_curr_app &&
                    l_index < ((plc_app_abi_t *)app.data.plc_curr_app)->l_sz) {
                  sendSysex(
                      FirmataStream, pb_firmata_cmd_FM_SET_LOC_TAB, sizeof(plc_loc_tbl_t),
            ...
            case pb_firmata_cmd_FM_GET_DBG_SIZE:
              if (app.data.plc_curr_app) {
                sendSysex(FirmataStream, pb_firmata_cmd_FM_GET_DBG_SIZE, 4,
                          (byte *)&((plc_app_abi_t *)app.data.plc_curr_app)
                              ->data->size_dbgvardsc);
              } else {
            ...
            case pb_firmata_cmd_FM_GET_DBG:
              ...
              if (argc == 4) {
                ...
                if (app.data.plc_curr_app &&
                    l_index <
                        ((plc_app_abi_t *)app.data.plc_curr_app)->data->size_dbgvardsc) {
                  len = (int)fill_dbg(
                      ((plc_app_abi_t *)app.data.plc_curr_app)->data->dbgvardsc,
                      (int)l_index, argv);
                }
              }
            ...
            case pb_firmata_cmd_FM_SET_DBG:
              ...
              if (argc > 5) {
                ...
                if (app.data.plc_curr_app &&
                    l_index <
                        ((plc_app_abi_t *)app.data.plc_curr_app)->data->size_dbgvardsc) {
                  set_dbg(((plc_app_abi_t *)app.data.plc_curr_app)->data->dbgvardsc,
                          l_index, &argv[4], argc - 4);
                  len = (int)fill_dbg(
                      ((plc_app_abi_t *)app.data.plc_curr_app)->data->dbgvardsc,
                      (int)l_index, argv);
                }
        ...
        case pb_firmata_cmd_FM_GET_CPU_SN:
            sendSysex(FirmataStream, pb_firmata_cmd_FM_GET_CPU_SN, 12,
                      (byte *) rte.data.sn);
            break;
#endif
        ...
        case pb_firmata_cmd_FM_WRITE_MEM:
            if (argc > 6) {
                ...
                sendSysex(FirmataStream, pb_firmata_cmd_FM_WRITE_MEM, len,
                          (byte *) indexv);
            }
            break;
#ifndef THIS_IS_BOOTLOADER
            case pb_firmata_cmd_FM_READ_VALUE:
                ...
                if ((argc == 8 && argv[0] <= REGION_HOLDER) &&
                    ...
                    *(uint32_t *) &sendBuffer[2] = indexv;
                    ...
                    sendSysex(FirmataStream, pb_firmata_cmd_FM_READ_VALUE_REP, len + 6,
                              (byte *) sendBuffer);
                    // free(buffer);
                } else {
                    len = -1;
                    sendSysex(FirmataStream, pb_firmata_cmd_FM_READ_VALUE_REP, 2,
                              (byte *) &len);
                }
                break;
            case pb_firmata_cmd_FM_WRITE_VALUE:
                if (argc > 7) {
                    ...
                    sendSysex(FirmataStream, pb_firmata_cmd_FM_WRITE_VALUE_REP, 7,
                              (byte *) argv);
                }
                break;
            case pb_firmata_cmd_FM_READ_BIT:
                ...
                if (len > 0) {
                    ...
                    *(uint32_t *) argv = indexv;
                    ...
                    sendSysex(FirmataStream, pb_firmata_cmd_FM_READ_BIT_REP, len / 8 + 6,
                              (byte *) buffer);
                    free(buffer);
                }
                break;
            ...
            case pb_firmata_cmd_FM_GET_LOCATION:
              ...
              sendSysex(FirmataStream, pb_firmata_cmd_FM_GET_LOCATION, len,
                        (byte *)buf_fgl);
              free(buf_fgl);
              break;
...
//                    len = serial->data.tx_buf_size + 4;
            ...
            case pb_firmata_cmd_FM_LFS_LS:
              if (argc > 8) {
                ...
                len = kfs.dir_buf(pb_firmata_cmd_FM_LFS_LS, (const char *)&argv[8], since,
                                  size, this, FirmataStream);
              }
              break;
...
int mFirmata::read_rte_const(mFirmata *mf, nStream *pStream, pb_cmd cmd) {
    ...
    pb_ostream_t stream =
            pb_ostream_from_buffer(mf->sendBuffer, FIRMATA_BUFFER_SZ);
    ...
    mf->sendSysex(pStream, pb_firmata_cmd_FM_PROTOBUF, stream.bytes_written,
                  mf->sendBuffer);
    return 0;
}
...
int mFirmata::get_tsdb_info(mFirmata *mf, nStream *pStream, pb_cmd cmd) {
    ...
    int res = 0;
    pb_tsdb_infos infos;
    auto *info = (pb_tsdb_info *)malloc(sizeof(pb_tsdb_info) * rteConst.tsdb_nrs);
...
bool mFirmata::buf_write(pb_ostream_t *stream, const pb_byte_t *buf,
                         size_t count) {
...
#ifndef THIS_IS_BOOTLOADER
    pb_module_info *info =
            (pb_module_info *) malloc(sizeof(pb_module_info) * rteConst.module_nrs);
