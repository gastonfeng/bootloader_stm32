    ...
    if (parsingSysex) {
        if (inputData == END_SYSEX) {
            ...
            if (crc == crc1) {
                ...
                logger.error("crc error 0x%x 0x%x", crc, crc1);
            }
            stream->flag &= ~FLAG_SYSEX;
        } else {
...
void mFirmata::sysexCallback(nStream *FirmataStream, byte command, uint16_t argc, byte *argv) {
    ...
    switch (command) {
        ...
        case CB_GET_RTE_VERSION:
            {
            pb_rte pb;
            pb.sn=plc_var.info.sn;
            pb.major=plc_var.info.major;
            pb.minor=plc_var.info.minor;
            pb.build=plc_var.info.build;
            pb.feature=plc_var.info.feature;
            pb.serial_nrs=plc_var.info.serial_nrs;
            pb.tx_buffer_size=FirmataStream->tx_max_size()*7/8;
    ...
    mf->sendSysex(FirmataStream, CB_GET_RTE_VERSION, ostream.bytes_written, buffer);
    free(buffer);
            }
            break;
#if defined(RTE_APP) || defined(PLC)
...
state&=~0x3;
        ...
        case FM_GET_CPU_SN:
            sendSysex(FirmataStream, FM_GET_CPU_SN, 12, plc_var.info.sn);
            break;
#endif
        case FM_READ_MEM:
            indexv = 0;
        ...
        case FM_REMOVE_FILE:
            ...
            sendSysex(FirmataStream, FM_REMOVE_FILE, 4, (byte *) &len);
            break;
#endif
        default:
            len = -1;
